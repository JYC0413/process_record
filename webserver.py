        ...
        for i, audio in enumerate(merged_audio_group):
            ...
            if i == 0:
                prompt = ('请将我上传的音频文件转录为文本，并精确标注每句话的起始时间、结束时间以及说话人。**转录内容必须严格按照语音流逐字输出，即使是中英文、或多国语言混合（语码转换）的句子，也必须完整保留所有语言的词汇，不得忽略任何一个词或部分。**'
                          ...
                          '时间格式严格遵循秒数（例如：86.160）,时间戳的精度至关重要，转录系统必须尽最大努力保持时间同步，最小化长音频转录中的时间漂移或累积误差转录结果中，任何一句文本的持续时间（end_time 减去 start_time）必须与语音内容的实际长度相匹配。严禁将长时间的静音或空白时间计入句子的持续时间，speaker 请用 "Speaker 1" 等标识。')
                ...
                prompt = (f'**这是长音频的第 {i+1} 个片段。请严格保持说话人编号与提供的参考音频一致。**\n'
                          ...
                          '请将音频转录为文本，并精确标注每句话的起始时间、结束时间以及说话人。时间格式严格遵循秒数（例如：86.160）,时间戳的精度至关重要，转录系统必须尽最大努力保持时间同步，最小化长音频转录中的时间漂移或累积误差任何一句文本的持续时间（end_time 减去 start_time）必须与语音内容的实际长度相匹配。严禁将长时间的静音或空白时间计入句子的持续时间，**转录内容必须严格按照语音流逐字输出，即使是中英文、或多国语言混合（语码转换）的句子，也必须完整保留所有语言的词汇，不得忽略任何一个词或部分。** 输出格式必须是 JSON：包含 "transcriptions" 数组以及 "speaker_map"。')
